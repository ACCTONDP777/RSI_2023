...
var tableFirstColumn = [{ name: 'MTL_GROUP', data: 'MTL_GROUP', width: '55px' },
    ...
    { name: 'SOURCER_OWNER', data: 'SOURCER_OWNER', width: '140px', className: 'text-ellipsis-owner', searchable: false },
    {
        ...
        render: function (data, type, row) {
            ...
            if ((priceidentity === 'Part_No' || hasSubTotal) && data != null)
                return "<p class='custom_tooltip'>" + new Intl.NumberFormat('en-US', { maximumFractionDigits: 8 }).format(data) + "</p>";
            return null;
        }
    },
    {
        ...
        render: function (data, type, row) {
            ...
            var hasSubTotal = isMaterialParts || isPartsGroup || isMaterialGroup;
            if ((priceidentity === 'Part_No' || hasSubTotal) && data != null)
                return new Intl.NumberFormat('en-US', { maximumFractionDigits: 8 }).format(data);
            return null;
        }
    },
    {
    ...
    {
        name: 'Sourcer_Amount', data: null, defaultContent: '', width: '70px', className: 'text-right', searchable: false,
        render: function (data, type, row) {
            if (row.ISMODIFY !== 'Y')
                return null;
            ...
            if ((priceidentity === 'Part_No' || hasSubTotal) && (row.USAGE != null && row.PRICE != null))
                return new Intl.NumberFormat('en-US', { maximumFractionDigits: 8 }).format(row.USAGE * row.PRICE);
            return null;
        }
    },
    {
        name: 'PM_Amount', data: null, defaultContent: '', width: '70px', className: 'text-right', searchable: false,
        render: function (data, type, row) {
            if (row.ISMODIFY !== 'Y')
                return null;
            ...
            if ((priceidentity === 'Part_No' || hasSubTotal) && (row.USAGE != null && row.PRICE_PM != null))
                return new Intl.NumberFormat('en-US', { maximumFractionDigits: 8 }).format(row.USAGE * row.PRICE_PM);
            return null;
        }
    }];
...
var tableOtherColumn = [
    ...
    {
        name: 'Download File', data: 'FILE_STATUS', width: '55px', className: 'text-center', defaultContent: '', searchable: false,
        render: function (data, type, row) {
            if (data === "Y") {
...
$(function () {
    ...
    if (phase_id == '40') {
        var table_ajax = {
            url: '/RSI/Sourcer/Sourcer_TableView',
            method: 'POST',
            data: {
                rsi_no: rsi_no,
                part_type: part_type,
